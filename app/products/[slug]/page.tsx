...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata>{const {slug}=await params;const product=getProduct(slug);const image=`/product-${slug}.webp`;return product?{title:`${product.name}｜广州山江钢结构`,description:product.description,alternates:{canonical:`/products/${slug}`},openGraph:{title:product.name,description:product.description,images:[image]},twitter:{card:"summary_large_image",title:product.name,description:product.description,images:[image]}}:{};}
...
export default async function ProductPage({params}:{params:Promise<{slug:string}>}){
  ...
  return <main className="detail">
    <header className="detailTop"><a className="brand" href="/"><b>SJ</b><span>广州山江钢结构<small>SHANJIANG STEEL STRUCTURE</small></span></a><div><a href="/#products">全部产品</a><a className="detailCta" href="#inquiry">获取报价 →</a></div></header>
